...
{
    if (is_ambiguous(state->expanded_value) && *num_tokens > 0 && (tokens[*num_tokens - 1].type == TOKEN_REDIRECT_OUT 
...
int add_to(const char **p, Token *tokens, int *num_tokens, QuoteWordParserState *state)
{
    int i;
    i = 0;
    ...
    if (*p > state->start) 
    {
    state->expanded_value = expand_variable(state->start, *p - state->start,state->env);
...
void handle_quotes_and_words(const char **p, Token *tokens, int *num_tokens, QuoteWordParserState *state) 
{
    ...
    state->buffer[state->buffer_index] = '\0';
    if (state->buffer_index > 0 && state->flag == 0) // Only add token if buffer is not empty
